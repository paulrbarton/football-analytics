...
class UnderstatAPIScraper:
    ...
    async def get_team_matches(self, team_name: str, season: str) -> pd.DataFrame:
        ...
        try:
            # Get team results
            results = await self.understat.get_team_results(team_name, int(season))
    ...
    async def get_league_matches(self, season: str) -> pd.DataFrame:
        ...
        """
        try:
            matches = await self.understat.get_league_results("EPL", int(season))
    ...
    async def get_all_teams_data(self, season: str) -> pd.DataFrame:
        ...
        tasks = [
            self.get_team_matches(team, season) for team in self.PREMIER_LEAGUE_TEAMS
        ]
